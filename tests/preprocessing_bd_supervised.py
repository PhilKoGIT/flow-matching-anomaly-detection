"""
Preprocessing Pipeline für Business Dataset - SUPERVISED (Semi-Supervised)
Kompatibel mit dem Evaluator (load_dataset Funktion)

Split: 50% normale Daten für Training, Rest + alle Anomalien für Test
(Wie im Original-Evaluator für semi-supervised)
"""
...
def prepare_data_supervised():
    """
    Lädt und preprocessed das Business Dataset für semi-supervised Anomaly Detection.
    
    Split-Logik (wie im Original-Evaluator):
    - Normale Daten werden 50/50 gesplittet
    - Training: 50% der normalen Daten (KEINE Anomalien)
    - Test: Restliche 50% normale + ALLE Anomalien
    
    Returns:
        X_train_df: DataFrame mit Features
        X_test_df: DataFrame mit Features
        y_train: Series mit Labels (alle 0)
    ...
    # Split: Normale vs Anomalien
    normal_mask = y == 0
    anomaly_mask = y == 1
    
    df_normal = df[normal_mask].reset_index(drop=True)
    df_anomaly = df[anomaly_mask].reset_index(drop=True)
    
    y_normal = y[normal_mask].reset_index(drop=True)
    y_anomaly = y[anomaly_mask].reset_index(drop=True)
    
    # 50/50 Split der normalen Daten (chronologisch)
    split_idx = int(len(df_normal) * 0.5)
    
    df_train = df_normal.iloc[:split_idx].copy()
    y_train = y_normal.iloc[:split_idx].copy()
    
    # Test: Restliche normale + alle Anomalien
    df_test_normal = df_normal.iloc[split_idx:].copy()
    y_test_normal = y_normal.iloc[split_idx:].copy()
    
    df_test = pd.concat([df_test_normal, df_anomaly], ignore_index=True)
    y_test = pd.concat([y_test_normal, y_anomaly], ignore_index=True)
    
    # Test nach Datum sortieren
    df_test = df_test.sort_values('date_post').reset_index(drop=True)
    y_test = y_test.iloc[df_test.index].reset_index(drop=True)
    
    print(f"Train (only normal): {len(df_train)} samples")
    print(f"Test: {len(df_test)} samples ({y_test.sum()} anomalies = {y_test.mean()*100:.2f}%)")
...
if __name__ == "__main__":
    X_train_df, X_test_df, y_train, y_test, train_map, test_map, features = prepare_data_supervised()
    
    print("\n=== SUPERVISED PREPROCESSING COMPLETE ===")
